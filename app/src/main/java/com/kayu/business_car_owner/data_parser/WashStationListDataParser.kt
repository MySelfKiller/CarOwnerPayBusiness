 ...
 */
class WashStationListDataParser : BaseParse<Any?>() {
        ...
        if (state == 1) {
            ...
            if (null != dataJson) {
                ...
                if (null != listJson && listJson.length() > 0) {
                    val listArray = ArrayList<WashStationBean>()
                    for (x in 0 until listJson.length()) {
                        val stationBean =
                            GsonHelper.fromJson(listJson[x].toString(), WashStationBean::class.java)
                        if (stationBean != null) {
