...
import com.kayu.business_car_owner.ui.income.IncomeDetailedData
import org.json.JSONObject
import java.lang.Exception

class IncomeDetailedParser : BaseParse<Any?>() {
    ...
    override fun parseJSON(handler: Handler, jsonStr: String, dataVersion: Double): ResponseInfo {
        ...
        if (null != dataObj) {
            ...
            if (null != noticeArr && noticeArr.length() > 0) {
                val dataList: MutableList<IncomeDetailedData> = ArrayList()
                for (x in 0 until noticeArr.length()) {
                    val obj = noticeArr[x] as JSONObject
                    val noticeData =
                        GsonHelper.fromJson(obj.toString(), IncomeDetailedData::class.java)
                    if (noticeData != null) {
